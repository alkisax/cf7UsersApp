...
// require('dotenv').config(); //δεν χρειάζετε γιατι έκανα σετινγσ στο πακατζ
...
describe("Requests for /api/users", ()=>{
  ...
  beforeAll(()=>{
    ...
    token = authService.generateAccessToken(user); // το παίρνω απο εδω
  });

  it('GET Returns all users', async ()=>{
    const res = await request(app)
      .get('/api/users')
      .set('Authorization', `Bearer ${token}`); // και εδώ το διαμορφώνω
